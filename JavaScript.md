...
- 原型模式：
  ...
  - > 每个创建的函数都有一个`prototype`的属性，这个属性是一个指针，指向一个对象，而这个对象的用途可以由包含可以由所有实例共享的属性和方法；通俗讲：每个函数都有`prototype`属性，`prototype`指向一个对象，该对象的属性和方法被所有实例共享；
    >
