...
- `Function`类型：函数是对象，函数名是指针；
  ...
  - 函数内部属性：
    ...
    - `prototype`：原型，指针，指向一个对象；该对象是包含可以由特定类型的所有实例共享的属性和方法；
...
- JS可以很方便的创建单个对象，但当希望创建一类对象时，往往会造成很多重复代码。有以下的解决方式：
  ...
  - ！任何函数，只要通过new操作符来调用，那它就可以作为构造函数，而任何函数，如果不同过new调用，和普通函数没有什么不同；

  - prototype  原型模式，通过prototype让所有实例共享属性和方法：
...
- `__proto__`：所有的对象都有属性`__proto__`,指向该对象的构造函数的原型对象；
- `prototype`：所有方法（方法也是对象）除了有属性`__proto__`，还有属性`prototype`，`prototype`指向该方法的原型对象；
- `constructor`：默认情况，所有的原型对象都会自动获得一个`constructor`属性，反向指向函数；
- 原型对象：包含可以由所有实例共享的属性和方法；（默认会有`__proto__`，`constructor`；当存在所有实例共享的属性和方法时，也会包含在原型对象中；
