...
- `session`：HTTP协议是无状态协议，本身不支持服务端保存客户端信息，由于无状态记忆的特点，此次连接无法得到上次连接的状态，`session`就是用来表示标识用户、跟踪用户的。在WEB开发中，服务器可以为每个用户浏览器创建一个会话对象（session对象），注意：一个浏览器独占一个`session`对象(默认情况下)。因此，在需要保存用户数据时，服务器程序可以把用户数据写到用户浏览器独占的`session`中，当用户使用浏览器访问其它程序时，其它程序可以从用户的`session`中取出该用户的数据，为用户服务。
  - `session`一般保存于服务器中；
